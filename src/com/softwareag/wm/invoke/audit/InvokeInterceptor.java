...
public class InvokeInterceptor implements InvokeChainProcessor {
	...
	public void process(@SuppressWarnings("rawtypes") Iterator chain, BaseService baseService, IData pipeline,
		...
		/**
		 * will be same as root context id if this service is top level!
		 */
		String parentContext = getParentContextId(status);
		String serviceName = getServiceName(baseService);

		try {
			if (chain.hasNext()) {

				if (chainPreProcessor(rootContextId, parentContext, serviceName, baseService, pipeline, status)) {
				...
				// service success
				chainPostProcessor(rootContextId, parentContext, serviceName, baseService, pipeline, status);
			}
		} catch (InvokeException error) {

			// service exceptions arrive here
			chainPostProcessor(rootContextId, serviceName, baseService, pipeline, error);
	...
	protected String getRootContextId(ServiceStatus status) {
		 ...
		 * This won't work SW6 id generated by skywalking cannot be formatted as
		 * UUID as so wm won't accept it
		 * 
		 * String id = getEtoETransactionId(); setRootContextIdForThread(id);
		 * 
		 * return id; } else { return Service.getCurrentActivationID(); }
		 */
	 ...
	 */
	protected boolean chainPreProcessor(String integrationId, String parentContext, String serviceName,
			BaseService baseService, IData pipeline, ServiceStatus status) {
		...
		if (serviceName.startsWith("wm.tn")) {
			...
			System.out.println(
					"Processing start " + serviceName + ", RCID: " + integrationId + ", PCID: " + parentContext);
...
//			didLog = E2ETracerObject.isE2EConfigured();
		}
		return didLog;
	}
	 ...
	 *
	 */
	protected void chainPostProcessor(String integrationId, String parentContext, String serviceName,
			BaseService baseService, IData pipeline, ServiceStatus status) {
		if (serviceName.startsWith("wm.tn")) {
			// TN is a special case, won't do this for any other subproducts,
			// honest!
			...
			System.out.println(
					"Processing completion " + serviceName + ", RCID: " + integrationId + ", PCID: " + parentContext);
